...
public class Correctable {
    ...
    public byte[] getValue(double nVotes, int nResponses){
        ...
        while (true) {
            try {
                acquire = block.tryAcquire(nResponses, ACQUIRETIMEOUT, TIMEUNIT);
            } catch (InterruptedException e) {
                e.printStackTrace();
            }
            if (!acquire) {
            ...
            if (state == CorrectableState.ERROR) {
                block.release(nResponses);
                return null;
            }
            block.release(nResponses);
            if (votes - nVotes >= Acceptor.THRESHOLD && responses >= nResponses) {
                return ret_value;
            }
        }
    }

    private byte[] getValue(Consistency consistency) {
        ...
        while (true) {
            ...
            // calculation of needed votes and responses should only happen
    ...
    public void update(RequestContext context, TOMMessage reply, double votes, int responses) {
        if (!isFinal()) {
            ...
            block.release(responses); // informs that a response was received
    ...
    private void checkFinal() {
        ...
        if (this.responses >= needed_responses && this.votes >= 2 * t * wMax + 1.0 + Acceptor.THRESHOLD) {
            close();
        }
    }
